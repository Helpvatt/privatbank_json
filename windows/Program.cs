...
namespace TerminalClient
{
    ...
    class TerminalService : BackgroundService
    {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            _logger.LogInformation("Testing connection to 192.168.0.164...");
            bool connected = await _terminalClient.ConnectAsync("192.168.0.164");
            if (!connected)
            {
                _logger.LogWarning("Initial connection to 192.168.0.164 failed. Continuing...");
            }
            else
            {
                _logger.LogInformation("Initial connection to 192.168.0.164 successful.");
            }
    ...
    class Program
    {
        static async Task Main(string[] args)
        {
            Log.Logger = new LoggerConfiguration()
                ...
                .WriteTo.File("C:\\Logs\\TerminalClient.log", rollingInterval: RollingInterval.Day)
                .CreateLogger();

            var host = Host.CreateDefaultBuilder(args)
                .UseWindowsService(options =>
                {
                    options.ServiceName = "TerminalClientService";
                })
                .ConfigureServices((context, services) =>
                {
                    ...
                    services.AddLogging(builder =>
                    {
                        ...
                        {
                            settings.SourceName = "TerminalClientService";
